...
function GatePass() {
  ...
  const fetchStatus = () => {
    axios.get(`http://10.234.199.171:3001/api/gate/status/${studentId}`)
      .then(res => setStatus(res.data.status))
      .catch(err => console.error(err));
  };

  const fetchLogs = () => {
  axios.get(`http://10.234.199.171:3001/api/student/logs/${studentId}`)
  ...
  const performCheckIn = async (qrData) => {
    setLoading(true);
    try {
      const res = await axios.post('http://10.234.199.171:3001/api/gate/log', {
